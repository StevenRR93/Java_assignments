...
public String teamMostPlayed() {
	Team mostplayed;
	if (teamCount == 0)
		return null;
	mostplayed= teams[0];
	for (int i=1; i<teamCount; i++){
		if (mostplayed.gamesPlayed() < teams[i].gamesPlayed())
			mostplayed= teams[i];
	}
	return mostplayed.getName();	
}
...
public Team[] undefeatedTeams() {
	...
	Team[] undefeatedTeams= new Team[c];
	int a=0;
	for (int i=1; i<teamCount; i++) {
		if (teams[i].getLosses() == 0){
		undefeatedTeams[a++]=teams[i];
	}
	}
	return undefeatedTeams;
}
...
public boolean havePlayedTogether(String teamA, String teamB){
	Team A=teamWithName(teamA);
	Team B=teamWithName(teamB);
	if (indexOf(A) >=0 && indexOf(B) >= 0)
	{
	if (gamesPlayed[indexOf(A)][indexOf(B)] != 0)
		return true;	
	else
		return false;	
	}
